...
`dicogis-cli` only, entrypoint `dicogis-cli`. CI's `docker_builder.yml`
workflow builds it, smoke-tests `--version`/`--help`, and pushes to
`ghcr.io/guts/dicogis` (tags: `X.Y.Z`/`X.Y` on releases, `edge` on `master`)
